...
public class TimeDiff extends Parent {
	...
	private static final Stop[] LINE_STOPS = new Stop[]
			{
	...
	private void createKeys() {
		for (int i = 0; i < Util.KEY_CODES.length; i++) {
			for (int j = 0; j < Util.KEY_CODES[0].length; j++) {
				...
				key.setOnMousePressed((MouseEvent me) -> {onKey(key);});
	...
	private void onOperation(Code code) {
